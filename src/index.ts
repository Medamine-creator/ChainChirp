...
import { marketCommands, marketCommandDescriptions } from "@/commands/market";
import { chainCommands, chainCommandDescriptions } from "@/commands/chain";
...
function showStyledVersion(): void {
  ...
  console.log("");
  console.log(
    `${PALETTE.cyan("┌")} ${PALETTE.heading("ChainChirp")} ${PALETTE.muted(`v${version}`)}`
  );
  console.log(`${PALETTE.cyan("│")}`);
  console.log(
    `${PALETTE.cyan("│")} ${symbol("bitcoin")} ${PALETTE.primary("Professional Bitcoin CLI")}`
  );
  console.log(
    `${PALETTE.cyan("│")} ${PALETTE.muted(`${platform} • node ${nodeVersion}`)}`
  );
  console.log(`${PALETTE.cyan("└")}`);
  console.log("");
}
...
function showStyledHelp(): void {
  ...
  console.log(`${PALETTE.cyan("│")}`);
  console.log(
    `${PALETTE.cyan("│")} ${PALETTE.primary("Professional Bitcoin ecosystem CLI")}`
  );
  console.log(
    `${PALETTE.cyan("│")} ${PALETTE.muted("Real-time data, elegant terminal UX")}`
  );
  console.log(`${PALETTE.cyan("└")}`);
  console.log("");
  console.log(`${PALETTE.heading("Usage")}`);
  console.log(
    `  ${PALETTE.muted("$")} ${PALETTE.primary("chainchirp")} ${PALETTE.cyan("<command>")} ${PALETTE.muted("[options]")}`
  );
  console.log("");
  console.log(`${PALETTE.heading("Market Commands")}`);
  console.log(
    `  ${PALETTE.cyan("price")}      ${PALETTE.muted("Current Bitcoin price and market data")}`
  );
  console.log(
    `  ${PALETTE.cyan("volume")}     ${PALETTE.muted("24h trading volume across exchanges")}`
  );
  console.log(
    `  ${PALETTE.cyan("change")}     ${PALETTE.muted("Price changes over multiple periods")}`
  );
  console.log(
    `  ${PALETTE.cyan("highlow")}    ${PALETTE.muted("Daily and all-time price records")}`
  );
  console.log(
    `  ${PALETTE.cyan("sparkline")}  ${PALETTE.muted("ASCII price charts and trends")}`
  );
  console.log("");
  console.log(`${PALETTE.heading("Network Commands")}`);
  console.log(
    `  ${PALETTE.cyan("block")}      ${PALETTE.muted("Latest blocks and blockchain data")}`
  );
  console.log(
    `  ${PALETTE.cyan("mempool")}    ${PALETTE.muted("Transaction pool and congestion")}`
  );
  console.log(
    `  ${PALETTE.cyan("fees")}       ${PALETTE.muted("Transaction fee recommendations")}`
  );
  console.log(
    `  ${PALETTE.cyan("hashrate")}   ${PALETTE.muted("Network security and difficulty")}`
  );
  console.log(
    `  ${PALETTE.cyan("halving")}    ${PALETTE.muted("Next halving countdown and rewards")}`
  );
  console.log("");
  ...
  console.log(
    `  ${PALETTE.muted("$")} ${PALETTE.primary("chainchirp price")} ${PALETTE.muted("--detailed")}`
  );
  console.log(
    `  ${PALETTE.muted("$")} ${PALETTE.primary("chainchirp fees")} ${PALETTE.muted("--watch --interval 15")}`
  );
  console.log(
    `  ${PALETTE.muted("$")} ${PALETTE.primary("chainchirp sparkline")} ${PALETTE.muted("--width 60 --timeframe 7d")}`
  );
  console.log(
    `  ${PALETTE.muted("$")} ${PALETTE.primary("chainchirp price")} ${PALETTE.muted("--currency eur --json")}`
  );
  console.log("");
  console.log(
    `${PALETTE.muted("Learn more:")} ${PALETTE.cyan("https://github.com/tristanbietsch/chainchirp")}`
  );
  console.log("");
}
...
cli.option(
  "--json",
  "Output data in JSON format for automation and integration"
);
cli.option("--watch", "Enable real-time watch mode with automatic updates");
cli.option(
  "--interval <seconds>",
  "Set watch mode update interval in seconds",
  { default: 30 }
);
cli.option(
  "--currency <currency>",
  "Set currency for price display (usd, eur, gbp, jpy, btc, eth, sats)",
  { default: "usd" }
);
...
cli
  ...
  .option(
    "--detailed",
    "Show comprehensive market data including 24h high/low, volume, market cap, and all-time records"
  )
  .action(async (options) => {
    try {
      if (options.detailed) {
        const { detailedPriceCommand } = await import(
      ...
      console.error(
        chalk.red("✕ Price command failed:"),
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Volume command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Change command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ HighLow command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .option(
    "--timeframe <timeframe>",
    "Time period for chart (1h, 24h, 7d, 30d) [default: 24h]"
  )
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Sparkline command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Block command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Mempool command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Hashrate command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
...
cli
  ...
  .action(async (options) => {
    try {
      ...
      console.error(
        chalk.red("✕ Halving command failed:"),
        error instanceof Error ? error.message : "Unknown error"
      );
      process.exit(1);
    }
